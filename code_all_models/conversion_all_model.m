...
% mos hourly
hourly_mos_train.Phat = pv_conversion(hourly_mos_train,beta);
hourly_mos_test.Phat = pv_conversion(hourly_mos_test,beta);

% kf3
hourly_kf3_train.Phat = pv_conversion(hourly_kf3_train,beta);
hourly_kf3_test.Phat = pv_conversion(hourly_kf3_test,beta);
...
% mos hourly
[hourly_mos_train.perf_p,hourly_mos_train.perf_p_specific]=performance_index_pv(hourly_mos_train.Phat,solar_train.P,install_cap);
[hourly_mos_test.perf_p,hourly_mos_test.perf_p_specific]=performance_index_pv(hourly_mos_test.Phat,solar_test.P,install_cap);

% hourly kf3
[hourly_kf3_train.perf_p,hourly_kf3_train.perf_p_specific]=performance_index_pv(hourly_kf3_train.Phat,solar_train.P(horizon*2+1:end),install_cap);
[hourly_kf3_test.perf_p,hourly_kf3_test.perf_p_specific]=performance_index_pv(hourly_kf3_test.Phat,solar_test.P(horizon*2+1:end),install_cap);
...
save(savefile_test,'wrf_test','persis_test','lorenz_test','diagne_test','pelland_test',...
    'hourly_mos_test','daily_mos_test','hourly_kf3_test','daily_kf1a_test');

save(savefile_train,'wrf_train','persis_train','lorenz_train','diagne_train','pelland_train',...
    'hourly_mos_train','daily_mos_train','hourly_kf3_train','daily_kf1a_train');
