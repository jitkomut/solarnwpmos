...
%% HOURLY MODEL (not reported in the paper). Uncomment this section if the model is tested.
% model: WRF+MOS
% y(t) = b1*Iwrf(t) + b2*RHwrf(t) +  b3*Twrf(t) + b4*SZA(t)
% C(t) = [Iwrf(t) RHwrf(t) Twrf(t) SZA(t)]
% beta(t) = [b1 b2 b3 b4]^T
% dimension: y(T,1), C(T,p), beta(p,1); T = number of samples, p = number of predictors
% Note: train/test.time is timestamp of measurement
% training data for MOS
hourly_mos_train.info.name='MOS';
hourly_mos_train.info.model='y(t) = b1*Iwrf(t) + b2*RHwrf(t) +  b3*Twrf(t) + b4*SZA(t)';
hourly_mos_train.info.unit=unit;
hourly_mos_train.info.description={'C(t) = [Iwrf(t) RHwrf(t) Twrf(t) SZA(t)], beta(t) = [b1 b2 b3 b4]^T',...
                          'dimension: y(T,1), C(T,p), z(p,1); T = number of samples, p = number of predictors'};
hourly_mos_train.C=[solar_train.Iwrf solar_train.RHwrf solar_train.Twrf solar_train.SZA];
hourly_mos_train.y=solar_train.I;
hourly_mos_train.Twrf=solar_train.Twrf*10;
hourly_mos_train.beta=hourly_mos_train.C\hourly_mos_train.y;
hourly_mos_train.resid=hourly_mos_train.y-(hourly_mos_train.C*hourly_mos_train.beta);
hourly_mos_train.time=reshape(solar_train.time,horizon,[])';
[T,p ] = size(hourly_mos_train.C); 
hourly_mos_train.covbeta=(1/(T-p))*norm(hourly_mos_train.resid)^2*((hourly_mos_train.C'*hourly_mos_train.C)\eye(p));
% test data for MOS
hourly_mos_test.info.name='MOS';
hourly_mos_test.info.model='y(t) = b1*Iwrf(t) + b2*RHwrf(t) +  b3*Twrf(t) + b4*SZA(t)';
hourly_mos_test.info.unit=unit;
hourly_mos_test.info.description={'C(t) = [Iwrf(t) RHwrf(t) Twrf(t) SZA(t)], beta(t) = [b1 b2 b3 b4]^T',...
                          'dimension: y(T,1), C(T,p), beta(p,1); T = number of samples, p = number of predictors'};
hourly_mos_test.C=[solar_test.Iwrf solar_test.RHwrf solar_test.Twrf solar_test.SZA];
hourly_mos_test.y=solar_test.I;
hourly_mos_test.Twrf=solar_test.Twrf*10;
hourly_mos_test.beta=hourly_mos_train.beta;
hourly_mos_test.time=reshape(solar_test.time,horizon,[])';

%% Hourly model: WRF+MOS+KF3 (This model is not reported in the paper)
% [y(t) y(t-1) ... y(t-h+1)]^T = blkdiag([C(t) C(t-1) ... C(t-h+1)])*[z(t) z(t-1) ... z(t-h+1)]^T
% A = [I(p,p) 0(p,p(h-1)); I(p(h-1),p(h-1)) 0(p(h-1),p)]
% C(t) = blkdiag([Iwrf(t) RHwrf(t) Twrf(t) SZA(t)],[Iwrf(t-1) RHwrf(t-1) Twrf(t-1) SZA(t-1)], ..., [Iwrf(t-h+1) RHwrf(t-h+1) Twrf(t-h+1) SZA(t-h+1)]
% beta(t) = [b1 b2 b3 b4]^T
% z(t) = [beta(t) beta(t-1) ... beta(t+h-1)]^T  
% W = blkdiag(1/1000000*diag(beta),zeros(p(h-1),p(h-1))), V = cov(v), v = [y(t) - yhat_mos(t), y(t-1) - yhat_mos(t-1), ... , y(t-h+1) - yhat_mos(t-h+1)], z(1|0) = [zLS zLS ... zLS]^T (h copies) which is obtained from MOS, P(1|0) = Kronecker product of sigma*inv(X^TX)
% dimension: A(ph,ph), C(h,ph,T), W(ph,ph), V(h,m), y(h,T), z(ph,1), P(ph,ph); h = forecast horizon, m = number of outputs, p = number of predictors, T = number of samples
% Note: train/test.time is timestamp of measurement

h=horizon; p=size(hourly_mos_train.C,2); 
% train data
hourly_kf3_train.info.name='MOS+KF3';
hourly_kf3_train.info.model='[y(t), y(t-1), ..., y(t-h+1)]^T = blkdiag([C(t) C(t-1) ... C(t-h+1)])*[z(t) z(t-1) ... z(t-h+1)]^T';
hourly_kf3_train.info.unit=unit;
hourly_kf3_train.info.description={'A = [I(p,p) 0(p,p(h-1)); I(p(h-1),p(h-1)) 0(p(h-1),p)], C(t) = blkdiag([Iwrf(t) RHwrf(t) Twrf(t) SZA(t)],[Iwrf(t-1) RHwrf(t-1) Twrf(t-1) SZA(t-1)], ..., [Iwrf(t-h+1) RHwrf(t-h+1) Twrf(t-h+1) SZA(t-h+1)], beta(t) = [b1 b2 b3 b4]^T, z(t) = [beta(t) beta(t-1) ... beta(t+h-1)]^T, W = blkdiag(1/1000000*diag(beta),zeros(n(h-1),n(h-1))), V = cov(v), v = [y(t) - yhat_mos(t), y(t-1) - yhat_mos(t-1), ... , y(t-h+1) - yhat_mos(t-h+1)], z(1|0) = [zLS zLS ... zLS]^T (h copies) which is obtained from MOS, P(1|0) = Kronecker product of sigma*inv(X^TX)',...
                            'dimension: A(ph,ph), C(h,ph,T), W(ph,ph), V(h,1), y(h,T), z(ph,1), P(ph,ph); h = forecast horizon, p = number of predictors, T = number of samples'};
hourly_kf3_train.z_init=repmat(hourly_mos_train.beta,h,1);
hourly_kf3_train.A=[eye(p) zeros(p,p*(h-1)); eye(p*(h-1)) zeros(p*(h-1),p)];
hourly_kf3_train.W=abs(blkdiag(1/1000000*diag(hourly_mos_train.beta),zeros(p*(h-1),p*(h-1))));
hourly_kf3_train.V=xcorr(hourly_mos_train.resid-mean(hourly_mos_train.resid),h-1,'unbiased');
hourly_kf3_train.V=toeplitz(hourly_kf3_train.V(h:end));
hourly_kf3_train.P_init=kron(ones(h),hourly_mos_train.covbeta);
hourly_kf3_train.C=construct_delay([solar_train.Iwrf solar_train.RHwrf solar_train.Twrf solar_train.SZA],solar_train.time,h); 
T=size(hourly_kf3_train.C,1);
hourly_kf3_train.C=mat2cell(hourly_kf3_train.C,ones(1,T), repmat(p,1,h));
hourly_kf3_train.C=construct_blkdiag_kf_hourly(hourly_kf3_train.C);
hourly_kf3_train.y=reshape(construct_delay(solar_train.I,solar_train.time,h)',h,[]);
hourly_kf3_train.Twrf=solar_train.Twrf(h*2+1:end)*10;
hourly_kf3_train.time=solar_train.time(2*h+1:end);  
% test data
hourly_kf3_test.info.name='MOS+KF3';
hourly_kf3_test.info.model='[y(t), y(t-1), ..., y(t-h+1)]^T = blkdiag([C(t) C(t-1) ... C(t-h+1)])*[z(t) z(t-1) ... z(t-h+1)]^T';
hourly_kf3_test.info.unit=unit;
hourly_kf3_test.info.description={'A = [I(p,p) 0(p,p(h-1)); I(p(h-1),p(h-1)) 0(p(h-1),p)], C(t) = blkdiag([Iwrf(t) RHwrf(t) Twrf(t) SZA(t)],[Iwrf(t-1) RHwrf(t-1) Twrf(t-1) SZA(t-1)], ..., [Iwrf(t-h+1) RHwrf(t-h+1) Twrf(t-h+1) SZA(t-h+1)], beta(t) = [b1 b2 b3 b4]^T, z(t) = [beta(t) beta(t-1) ... beta(t+h-1)]^T, W = blkdiag(1/1000000*diag(beta),zeros(n(h-1),n(h-1))), V = cov(v), v = [y(t) - yhat_mos(t), y(t-1) - yhat_mos(t-1), ... , y(t-h+1) - yhat_mos(t-h+1)], z(1|0) = [zLS zLS ... zLS]^T (h copies) which is obtained from MOS, P(1|0) = Kronecker product of sigma*inv(X^TX)',...
                            'dimension: A(ph,ph), C(h,ph,T), W(ph,ph), V(h,1), y(h,T), z(ph,1), P(ph,ph); h = forecast horizon, p = number of predictors, T = number of samples'};
hourly_kf3_test.z_init=hourly_kf3_train.z_init;
hourly_kf3_test.A=hourly_kf3_train.A;
hourly_kf3_test.W=hourly_kf3_train.W;
hourly_kf3_test.V=hourly_kf3_train.V;
hourly_kf3_test.P_init=kron(ones(h),hourly_mos_train.covbeta);
hourly_kf3_test.C=construct_delay([solar_test.Iwrf solar_test.RHwrf solar_test.Twrf solar_test.SZA],solar_test.time,h); 
T=size(hourly_kf3_test.C,1);
hourly_kf3_test.C=mat2cell(hourly_kf3_test.C,ones(1,T), repmat(p,1,h));
hourly_kf3_test.C=construct_blkdiag_kf_hourly(hourly_kf3_test.C);
hourly_kf3_test.y=reshape(construct_delay(solar_test.I,solar_test.time,h)',h,[]);
hourly_kf3_test.Twrf=solar_test.Twrf(h*2+1:end)*10;
hourly_kf3_test.time=solar_test.time(2*h+1:end);
