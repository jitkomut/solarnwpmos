...
%% Predict Ihat from proposed MOS hourly-step and daily-step
% hourly model (this model is not reported in the paper)
hourly_mos_train.Ihat = hourly_mos_train.C*hourly_mos_train.beta;
hourly_mos_test.Ihat = hourly_mos_test.C*hourly_mos_test.beta;
...
hourly_mos_train.Ihat=reshape(hourly_mos_train.Ihat,horizon,[])';
hourly_mos_test.Ihat=reshape(hourly_mos_test.Ihat,horizon,[])';
...
%% Residual (D,h)
% hourly model
hourly_mos_train.resid = reshape(hourly_mos_train.resid,horizon,[])';
hourly_mos_test.resid = hourly_mos_test.Ihat-reshape(solar_test.I,horizon,[])';
...
% if neglect the first two days
% persis_train.resid = persis_train.Ihat - reshape(solar_train.I(horizon*2+1:end),horizon,[])';
% persis_test.resid = persis_test.Ihat - reshape(solar_test.I(horizon*2+1:end),horizon,[])';
...
[hourly_mos_train.perf,hourly_mos_train.perf_specific] = performance_index(hourly_mos_train.Ihat,solar_train.I);
...
% [persis_train.perf,persis_train.perf_specific] = performance_index(persis_train.Ihat,solar_train.I);
...
[hourly_mos_test.perf,hourly_mos_test.perf_specific] = performance_index(hourly_mos_test.Ihat,solar_test.I);
...
% [persis_test.perf,persis_test.perf_specific] = performance_index(persis_test.Ihat,solar_test.I);

[persis_test.perf,persis_test.perf_specific] = performance_index(persis_test.Ihat(3:end,:),solar_test.I(horizon*2+1:end));

%% Run KF: KF3, KF1a, Diagne, and Pelland models  (KF1a is our model reported in the paper)
%% hourly-step MOS+KF3 (This model is not reported in the paper)
hourly_kf3_train = run_kf_hourly(hourly_kf3_train,tf);
hourly_kf3_train.time = reshape(hourly_kf3_train.time,h,[])'    
hourly_kf3_test = run_kf_hourly(hourly_kf3_test,tf);
hourly_kf3_test.time = reshape(hourly_kf3_test.time,h,[])'    
...
%% residual error
% kf hourly 
hourly_kf3_train.resid=cal_residual_error(hourly_kf3_train.Ihat,solar_train.I(horizon+1:end),horizon);
hourly_kf3_test.resid=cal_residual_error(hourly_kf3_test.Ihat,solar_test.I(horizon+1:end),horizon);
% diagne
...
%% performance index
% kf hourly
[hourly_kf3_train.perf,hourly_kf3_train.perf_specific] = performance_index(hourly_kf3_train.Ihat,solar_train.I(horizon*2+1:end));
[hourly_kf3_test.perf,hourly_kf3_test.perf_specific] = performance_index(hourly_kf3_test.Ihat,solar_test.I(horizon*2+1:end));
% diagne
...
save moskf_test_results wrf_test persis_test daily_mos_test hourly_mos_test hourly_kf3_test daily_kf1a_test lorenz_test diagne_test pelland_test 
save moskf_train_results wrf_train persis_train daily_mos_train hourly_mos_train hourly_kf3_train daily_kf1a_train lorenz_train diagne_train pelland_train
save solar_splitdata solar_train solar_test
